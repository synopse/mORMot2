...
// register mormot.lib.sspi WinX509Parse() on Windows as failover
var
  X509Parse: function(const Cert: RawByteString;
...
begin
  ...
  PeerInfo := ToText(c);
  Usage := TCryptCertUsages(c.Usage); // TWinCertUsages match ASN1/X509 16-bit
  NotBef := c.NotBefore;
  NotAft := c.NotAfter;
end;
{$endif OSWINDOWS}
...
begin
  ...
  {$ifdef OSWINDOWS}
  X509Parse := @WinX509Parse; // from mormot.lib.sspi
  {$endif OSWINDOWS}
end;
