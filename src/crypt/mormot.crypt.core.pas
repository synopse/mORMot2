...
type
  ...
  // AES-CTR with periodic reseeding from SHAKE-256 mixed entropy
  ...
  // - the key is periodically regenerated from several independent entropy
  // sources (Operating System, hardware and process state) mixed with SHAKE-256
  // in XOF mode, with a secret chaining state for forward secrecy
  ...
  public
    ...
    // - entropy is gathered from the OS via SHAKE-256, using GetEntropy() method
  ...
  // - will call /dev/urandom under POSIX, or ProcessPrng() API on Windows
  ...
  // from the "black-box" approach of the OS, give consistent randomness
