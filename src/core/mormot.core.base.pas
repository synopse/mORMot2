...
const
  ...
  // commit.sh script) - so won't be reset when minor is increased
...
type
  ...
  /// map a RawByteString/UnicodeString header as generated by the compiler
  ...
  /// map a dynamic array header as generated by the compiler
  TDynArrayRec = record
  ...
  /// store a fake RawUtf8 constant string with up to 7 chars as contiguous block
  TStrRecConst = packed record
    Header: TStrRec;
    TextLo, TextHi: cardinal;  // more convenient than [0..7] of AnsiChar
  end;
...
// - under Intel/AMD, will detect SSE2 and use it if available, reaching e.g.
...
/// retrieve some entropy bits, as used to seed our gsl_rng_taus2 TLecuyer
...
var
  ...
  // OS entropy - consider TAesPrng.GetEntropy() from mormot.crypt.core instead
...
type
  ...
  public
    ...
    /// prepare to append some bytes to the internal buffer but won't touch Size
    function Prepare(l: PtrInt): pointer;
      {$ifdef HASINLINE}inline;{$endif}
    /// prepare to append some bytes to the internal buffer and increase Size
    ...
    property Buffer: pointer
      read Store.buf;
    /// how many bytes have been stored via Add*() methods - can be reset to 0
    property Size: integer
      read Store.added write Store.added;
...
function TSynTempAdder.Prepare(l: PtrInt): pointer;
var
  new: PtrInt;
begin
  new := l + Store.added;
...
end; // return prepared destination buffer but keep Store.added untouched
