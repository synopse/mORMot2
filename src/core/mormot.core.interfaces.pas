...

{
  *****************************************************************************
   ...
   Implements SOLID Process via Interface types
    ...
    - TServiceMethodExecute for Method Execution from JSON
...
type
  ...
  protected
    ...
    function Implements(aInterface: PRttiInfo): boolean; virtual; abstract;
  end;
  {$M-}
...
type
  ...
  TServiceMethodExecuteEventStep = (
...
begin
  ...
  try
    W.Add('{');
    while (P <> nil) and GetNextFieldProp(P, arg) and
          (P <> nil) and (arg <> '') do
    begin
      ok := true;
...
begin
  // obfucated but very efficient expression once inlined
...
var
  ...
  begin
    ...
    raise EInterfaceFactory.CreateUTF8('%.FakeCall(%.%) failed: %', [self,
      fFactory.fInterfaceName, method^.URI, msg]);
  end;
...
var
  ...
  F: ^TInterfaceFactory;
begin
  if (aInterface = nil) or (aInterface^.Kind <> rkInterface) then
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if ((PARAMREG_LAST - reg + 1) * PTRSIZ) < SizeInStack then
          begin
          ...
          if SizeInStack > PTRSIZ then
            inc(reg, SizeInStack shr PTRSHR)
          else
            inc(reg);
          {$else}
          RegisterIdent := reg;
...
begin
  if (self = nil) or (fImplementation.ValueClass = nil) then
    result := ''
  else
...
begin
  ...
  else if (high(aInterfaces) = 0) and (high(aObjs) = 0) then
...
begin
  ...
  else if (wResults in aScope) and (method^.ArgsResultIndex >= 0) then
  begin
  ...
  begin
    ...
    begin
      ...
      if (ValueDirection <> smdConst) or
         (ValueType in [smvRecord, smvVariant]) then
       begin
        // pass by reference
...
function TServiceMethodExecute.ExecuteJsonFake(
...
begin
  ...
  try
    ...
    if (ArgsInputValuesCount <> 0) and (Par <> nil) then
    begin
      if Par^ in [#1..' '] then
        repeat
          inc(Par);
    ...
    if (Par = nil) and not ParObjValuesUsed then
    begin
        ...
        begin
          ...
          case ValueType of
            smvDynArray:
              begin
                if vIsObjArray in ValueKindAsm then
                  Res.AddObjArrayJSON(fValues[a]^, opt[ValueDirection = smdVar])
                else
                  Res.AddDynArrayJSON(fDynArrays[IndexVar].Wrapper);
                Res.Add(',');
              end;
          else
            AddJSON(Res, fValues[a], opt[ValueDirection = smdVar]);
          end;
        end;
      Res.CancelLastComma;
    end;
