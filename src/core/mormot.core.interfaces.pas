  ...
  begin
    ...
    for i := 1 to InterfaceFactoryCache.Count do
      with PGUID32(@F^.fInterfaceIID)^ do
        if (a = ga) and (b = GUID32.b)
          {$ifdef CPU32} and (c = GUID32.c) and (d = GUID32.d) {$endif} then
        begin
          result := F^;
          InterfaceFactoryCache.Safe.UnLock;
          exit;
...
begin
  nm := GetRttiInterface(aInterface, info);
  ...
  try // release server instance
    ...
    begin
      ...
      if C.InheritsFrom(EInterfaceFactory) or
        ...
        raise; // ignore most obvious low-level exceptions
    end;
  end;
  inherited;
end;
