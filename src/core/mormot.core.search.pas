...
begin
  ...
  try
    ...
    fZones.Capacity := n;
    for i := 0 to n - 1 do
      if reg.ReadReOpen(wrLocalMachine, [REGKEY, keys[i]]) then
      begin
        z.Clear;
        ...
        if reg.ReadReOpen(wrLocalMachine, [REGKEY, keys[i], '\Dynamic DST']) then
        begin
...
begin
  ...
  if not reg.ReadReOpen(wrLocalMachine, [REGKEY, z^.id], {reopen=}false) then
