...
type
  ...
  // - efAesCtc128/efAesCtc256 is TAesCtrCrc, so around 2.4GB/s on x86_64, with
...
begin
  ...
  aClient.RndA := fRndA;
  if fAlgo.auth <> authClient then
    if not Ecc256r1MakeKey(aClient.QE, fdE) then // ephemeral key
...
begin
  ...
  aServer.RndB := fRndB;
  if fAlgo.auth <> authServer then
    if not Ecc256r1MakeKey(aServer.QF, dF) then // ephemeral key
      raise EECCException.CreateUtf8('%.ComputeHandshake: MakeKey?', [self]);
  try
    result := sprInvalidPublicKey;
    if fAlgo.auth <> authServer then
