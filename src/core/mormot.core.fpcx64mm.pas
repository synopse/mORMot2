...
// - from our tests on Linux, this may be slower on high thread contention
...
// - from our tests on Linux, spinning is slower on high thread contention
{.$define FPCMM_PAUSE}
...
@ProcessPendingBin:
  ...
  mov eax, $100
  lock cmpxchg [rbx].TSmallBlockType.BinLocked, ah
  je @BinLocked
  pause
  mov eax, $100
  lock cmpxchg [rbx].TSmallBlockType.BinLocked, ah
...
@BinLocked:
  ...
  xor eax, eax
  mov byte ptr [rbx].TSmallBlockType.BinLocked, al
  pop rbx
  ret
@LockBlockType:
...
@AdjustGetMemMoveFreeMem:
  // New allocated size is the maximum of the requested size and minimum upsize
  cmp rdx, P
  cmova P, rdx
  push rdx
  call _GetMem
