...
begin
  ...
  v := pointer(Ctxt.fRouteValuePosLen);
  if v = nil then
     exit; // paranoid
  ...
  repeat
    ...
    if t[2] >= 0 then
    begin
      v := @Ctxt.fRouteValuePosLen[t[2] * 2];
      MoveFast(PByteArray(Ctxt.fUrl)[v[0]], p^, v[1]); // value [pos,len] pair
      inc(p, v[1]);
    end;
    t := @t[3];
    dec(n, 3)
...
begin
  ...
  // validate only URI-compatible chars (excluding inlined parameters after ?)
  p := u;
  if p = nil then
    exit;
  repeat
  ...
  if PosExChar('<', aFromUri) = 0 then
    n := Insert(aFromUri) as TUriTreeNode // a simple static route
  else
    // parse static..<param1>..static..<param2>..static into static/param nodes
    repeat
      GetNextItem(u, '<', item);
      full := full + item;
      n := Insert(full) as TUriTreeNode; // some static (Names = nil)
      if u = nil then
        break;
      GetNextItem(u, '>', item);
      if item = '' then
        raise EUriTree.CreateUtf8('Void <> in %.Setup(''%'')', [self, aFromUri]);
      if FindRawUtf8(names, item) >= 0 then
        raise EUriTree.CreateUtf8('Duplicated <%> in %.Setup(''%'')',
          [item, self, aFromUri]);
      AddRawUtf8(names, item);
      full := full + '<' + item + '>'; // avoid name collision with static
      n := Insert(full) as TUriTreeNode; // some param (Names <> nil)
      n.Data.Names := copy(names);
      if (u = nil) or
         (u^ = #0) then
...
begin
  ...
  if found <> nil then
    if Assigned(found.Data.OnRequest) then
