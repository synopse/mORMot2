...
function DocVariantFromResults(out Doc: TDocVariantData;
  Json: PUtf8Char; JsonLen: PtrInt;
  Options: TDocVariantOptions = JSON_FAST_FLOAT): boolean; overload;
...
function DocVariantFromResults(out Doc: TDocVariantData;
  const Json: RawUtf8; Options: TDocVariantOptions = JSON_FAST_FLOAT): boolean; overload;
...
// - a private copy of the incoming JSON buffer will be used before parsing
function DocVariantFromResults(out Doc: TDocVariantData;
  const Json: RawUtf8; aModel: TDocVariantModel): boolean; overload;
  {$ifdef HASINLINE} inline; {$endif}
...
function DocVariantFromResults(out Doc: TDocVariantData;
  Json: PUtf8Char; JsonLen: PtrInt; Options: TDocVariantOptions): boolean;
var
  fields, rows, capa, i, f: PtrInt;
  P, V: PUtf8Char;
  VLen: integer;
  wasstring: boolean;
  endofobj: AnsiChar;
...
begin
  ...
  begin
    ...
    Doc.Capacity := rows;
    Doc.SetCount(rows);
    dv := pointer(Doc.Values);
    for i := 1 to rows do
    begin
      val := dv^.InitFrom(proto, {values=}false); // names byref + void values
      for f := 1 to fields do
      begin
        GetJsonToAnyVariant(val^, P, @endofobj, @Options, false{double=options});
        if P = nil then
          exit;
        inc(val);
      end;
  ...
  begin
    ...
    P := proto.InitJsonInPlace(P, Options, @endofobj);
    if P = nil then
      exit;
    rows := 0;
    capa := 16;
    ...
    repeat
      ...
      begin
        ...
        GetJsonToAnyVariant({%H-}val^, P, @endofobj, @Options, false{double=options});
        if P = nil then
          break;
        inc(val);
      end;
...
function DocVariantFromResults(out Doc: TDocVariantData;
  const Json: RawUtf8; Options: TDocVariantOptions): boolean;
var
  tmp: TSynTempBuffer;
begin
  tmp.Init(Json);
  try
    result := DocVariantFromResults(Doc, tmp.buf, tmp.len, Options);
  finally
    tmp.Done;
  end;
end;
...
function DocVariantFromResults(out Doc: TDocVariantData;
  const Json: RawUtf8; aModel: TDocVariantModel): boolean;
begin
  result := DocVariantFromResults(Doc, Json, JSON_[aModel]);
end;
