...
{ Thin CLI consumer (Recommended Patterns B.6.1).

  This program is pure presentation: it parses arguments and calls the CQRS
  ports through ITaskManagerClient. It is identical regardless of topology —
  the backend it links decides whether those ports are reached in-process or
  over HTTP (location transparency, A.6).
...
begin
  ...
  WriteLn('');
  {$ifdef LOCAL_MODE}
  WriteLn('Mode: LOCAL (embedded SQLite)');
  {$else}
  WriteLn('Mode: REMOTE (HTTP client to localhost:8080)');
  {$endif}
end;
...
begin
  ...
  try
    // CreateClient is resolved from whichever backend unit is linked
    // (AppTaskManagerClientLocal or AppTaskManagerClientRemote).
