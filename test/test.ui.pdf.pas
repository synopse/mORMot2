...
begin
  ...
  if (SubLen = 0) or
    ...
    exit;
  P := Pointer(Text);
  PEnd := P + TextLen - SubLen;
  while P <= PEnd do
  begin
    if CompareMem(P, Pointer(SubStr), SubLen) then
    begin
      inc(Result);
      inc(P, SubLen);
    end
...
begin
  ...
  try
    ...
    SetString(PdfContent, PAnsiChar(PdfStream.Memory), PdfStream.Size);
    Check(CountOccurrences(RawByteString(EXPECTED_TEXT), PdfContent) <> 0,
...
begin
  ...
  try
    ...
    SetString(PdfContent, PAnsiChar(PdfStream.Memory), PdfStream.Size);
    Check(CountOccurrences('/Author(mORMot Test Framework)', PdfContent) <> 0,
      'PDF metadata author field is incorrect or missing');
    Check(CountOccurrences('/Title(PDF \(Metadata\) \\ Test)', PdfContent) <> 0,
      'PDF metadata title field is incorrect or missing');
    Check(CountOccurrences('/Subject(Testing PDF Metadata Fields)', PdfContent) <> 0,
      'PDF metadata subject field is incorrect or missing');
    Check(CountOccurrences('/Creator(mORMot2 tests)', PdfContent) <> 0,
      'PDF metadata creator field is incorrect or missing');
    Check(CountOccurrences('/Keywords(pdf, mORMot)', PdfContent) <> 0,
