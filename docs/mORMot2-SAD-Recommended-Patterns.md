...
> **Gotcha 3 — Pascal casing doesn't travel; pick a JSON convention.** `TRttiMap` copies between Pascal identifiers in memory, so casing never matters to `ToA`/`ToB`. The moment a DTO crosses the wire as JSON it does: mORMot serializes field names exactly as declared, so `TUserDto.Email`/`YearOfBirth` go out as `{"Email":…,"YearOfBirth":…}` — PascalCase, which most web consumers don't expect. Normalize it **once, in the `initialization` of the unit that declares the DTO** (so every server, client, and consumer that links the type gets the same wire contract), and **on the DTO type, never the `TOrm`** — `NameChangeCase` mutates that type's shared RTTI process-wide, and you don't want to reshape every ORM payload: `Rtti[TypeInfo(TUserDto)].Props.NameChangeCase(scLowerCaseFirst)` turns every field camelCase (`email`, `yearOfBirth`) — the default convention for new web APIs, and the one mORMot's own docs flag as *"aka camelCase, for API."* On FPC, place it **after** the `Rtti.RegisterFromText` those record DTOs already need (the fields don't exist to rename until registration builds them; on Delphi the order is irrelevant). Reach for `scSnakeCase` (`year_of_birth`) instead only when the surrounding system already standardized on it (Rails/Django backends). If just a field or two deviate, alias them by hand with `Props.NameChanges(['Email','contactEmail'])`; pass `NestedEnums` to `NameChangeCase` when enum *member* names must follow the same convention on the wire (see Gotcha 2).
